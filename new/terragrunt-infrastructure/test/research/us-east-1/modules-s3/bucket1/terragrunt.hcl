...
inputs = {
  name                   = "${local.name}-${local.environment}-${dependency.random_id.outputs.random_id}-bucket1"
  object_expiration_days = 2
  bucket_object_prefix   = "/"
  iam_policy_name        = "${local.name}-${local.environment}-${dependency.random_id.outputs.random_id}-bucket1-role"
  kms_arn                = null
  bucket_key             = true
  tags                   = local.tags
}